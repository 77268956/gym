...
class UserController extends Controller
{
    ...
    {
        $nombre = 'Jose Perez';
        $activeMembers = \App\Models\Cliente::where('estado', 'activo')->count();
        $monthlyRevenue = 42390; // mock
        $attendanceRate = 84.2; // mock
        $newSignups = \App\Models\Cliente::whereMonth('created_at', now()->month)->count();
        $totalClientes = \App\Models\Cliente::count();
        $membresiasPorVencer = \App\Models\Membresia::where('estado', 'activa')
            ->where('fecha_vencimiento', '<=', now()->addDays(7))
            ->count();
        $clientes = \App\Models\Cliente::with(['membresias' => function($q) {
